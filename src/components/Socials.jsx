...
const Socials = () => {
	return (
		<ul className="top-0 left-0 z-10 flex w-full flex-wrap items-center justify-center gap-2 px-12 py-2.5 lg:fixed lg:h-full lg:w-auto lg:flex-col lg:p-2.5">
			{socialsItem.map((item, index) => (
				<li key={index}>
					<a
						href={item.link}
						aria-label={item.ariaLabel}
						className="block transition-transform duration-300 hover:scale-105 focus:scale-105"
						target="_blank"
						rel="noopener noreferrer"
					>
						<div
							className="h-8 w-8 transition-colors duration-300 ease-linear"
