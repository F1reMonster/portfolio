...
function ProjectCard({ project, latestProjectDate }) {
	...
	return (
		...
		<a
			...
			className="group text-silver relative flex min-h-65 flex-col overflow-clip rounded-[.625rem] bg-white/50 shadow-[0_0_0_1px_rgba(255,255,255,0.2),_2px_2px_6px_rgba(0,0,0,0.8)] dark:shadow-[0_4px_12px_rgba(255,255,255,0.4),_0_0_20px_rgba(255,255,255,0.2)] dark:bg-black/20"
			target="_blank"
			rel="noopener noreferrer"
		>
			<div className="absolute top-0 left-0 z-1 flex items-center gap-1.5 text-base/none">
				{project.date === latestProjectDate && <div className="bg-crimson animate-blinked p-1">new</div>}
				<div className="bg-blue-gem p-1">{new Date(project.date * 1000).toISOString().split("T")[0]}</div>
			</div>
			...
			<div className="absolute inset-0 overflow-clip">
				...
				<img decoding="async" className="w-full" src={project.cover} alt={project.title} loading="lazy" onLoad={handleImageLoad} />
			</div>

			<div className="z-1 mt-36 flex-1 bg-white/10 backdrop-blur-lg transition-colors duration-100 dark:bg-black/30">
				<h4 className="text-silver font-montserrat bg-crimson p-1 text-center text-lg font-bold">{project.title}</h4>
